...
| [`publish-release`](publish-release/SKILL.md) | Publica una release en GitHub: sugiere la versión SemVer (siempre la confirma con el usuario), compone las notas en español desde el log, crea el tag `vX.Y.Z` y adjunta un ZIP instalable limpio construido por la propia skill (`gh release create`). | "publica una release", "crea la release vX.Y.Z", "sube la release a GitHub" |
