...
TestSuite::run('Hooks - registerTabs embebe summary_fields con campos base + additional, respetando summaryView (STORY tabla dinamica)', function (): void {
    ...
    $pdo->rows = [[
        ...
        'schema_json' => json_encode([
            'fields' => [
                ...
                'od_lens_sphere' => ['type' => 'number', 'required' => false, 'label' => 'OD · Contacto · Esfera', 'summaryView' => false],
            ...
            'relations' => [],
            'ui_field_order' => ['date', 'od_lens_sphere', 'notes', 'warnings'],
        ], JSON_UNESCAPED_UNICODE),
    ]];
    ...
    assertEquals(['date', 'od_lens_sphere', 'notes', 'warnings'], array_keys($byKey), 'summary_fields must preserve schema_json.fields declaration order');
    assertTrue($byKey['date']['summaryView'] === true, 'date has no explicit summaryView -> defaults to true');
    assertTrue($byKey['od_lens_sphere']['summaryView'] === false, 'od_lens_sphere summaryView must be honored as false');
    ...
    assertEquals(['date', 'od_lens_sphere', 'notes', 'warnings'], $tab['ui_field_order'], 'ui_field_order must be embedded verbatim from schema_json');
});
