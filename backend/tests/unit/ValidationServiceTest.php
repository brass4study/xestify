...
TestSuite::run('validate skips type/required checks for auto_generated fields but still allows the key (STORY 10.5)', function () use ($service): void {
    $schema = [
        'fields' => [
            'body' => ['type' => 'text', 'required' => true],
            'author_id' => ['type' => 'uuid', 'required' => false, 'auto_generated' => true],
            'stamp' => ['type' => 'timestamp', 'required' => true, 'auto_generated' => true],
        ],
    ];

    // A raw, non-UUID author_id and a missing (required!) stamp would both
    // normally fail — but their final value is server-resolved after
    // validation (ExtensionPluginContentService::resolveAutoGeneratedValue()),
    // so the client's raw input must not block the write.
    $result = $service->validate(['body' => 'hola', 'author_id' => 'not-a-uuid'], $schema);

    assertTrue($result->isValid(), 'auto_generated fields must not be type/required-checked');
});

TestSuite::run('validate still rejects an unknown field even when the schema has auto_generated fields', function () use ($service): void {
    $schema = [
        'fields' => [
            'body' => ['type' => 'text', 'required' => true],
            'stamp' => ['type' => 'timestamp', 'required' => false, 'auto_generated' => true],
        ],
    ];

    $result = $service->validate(['body' => 'hola', 'unexpected' => 'x'], $schema);
    $error = findValidationError($result->errors(), 'unexpected', 'unknown_field');

    assertFalse($result->isValid(), 'Expected invalid result for an undeclared field');
    assertEquals('Unknown field: unexpected', $error['message'] ?? null);
});
