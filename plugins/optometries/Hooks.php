...
namespace Xestify\plugins\optometries;

use PDO;
use Xestify\core\HookDispatcher;

/**
 * Hooks for the optometries plugin.
 *
 * Registers a registerTabs filter hook that injects an "Optometria" tab for
 * every ACTIVE instance of this plugin (plugin_name='optometries') whose own
 * target_entity configuration allows the entity currently being viewed. Same
 * pattern as plugins/comments/Hooks.php, plus (STORY 10.5) embedding each
 * instance's own schema.relations[] in the tab payload: PluginPanelRegistry
 * .build() only ever receives {endpoint, recordId, api}, so this is how
 * plugin.js learns which relations exist, their target_entity, and which UI
 * layer (od/os/general) each belongs to, without a second round-trip to
 * discover the plugin's own schema. Also embeds schema.fields[] whose
 * origin is 'additional' (fields added later via PluginConfig's "Añadir
 * campo", not part of the plugin's original schema.json) — buildDetailForm()
 * in plugin.js has no hand-written UI for those, so it renders them
 * generically by type instead of silently dropping them (bug found by the
 * user: a field added via PluginConfig had zero visual effect).
 */
final class Hooks
{
    ...
    private function resolvePriority(): int
    {
        ...
        $stmt = $this->pdo->prepare(
            "SELECT MIN(sort_order) AS min_sort_order
               FROM plugins
              WHERE manifest_json->>'name' = 'optometries'
    ...
    /**
     * Only fields with origin:'additional' — the 29 fields the plugin ships
