...
// `persons` already carries a large, ever-growing seeded/demo dataset in local dev,
...
// response and navigate to its edit URL directly (`#/entity/persons/:id`, the same
...
test.describe('Entity CRUD (persons)', () => {
  test('creates and edits a person record via its direct edit URL', async ({ page }) => {
    ...
    const [createResponse] = await Promise.all([
      page.waitForResponse((res) => res.url().includes('/entities/persons/records') && res.request().method() === 'POST'),
    ...
    await page.goto(`#/entity/persons/${recordId}`);
    await page.reload();
