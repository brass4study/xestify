...
| [03](03-backend-motor-plugins.md) — Motor de plugins | 13 | 2 | 11 | 0 |
...
| 03.02 | ✅ | Mayor | 3 tests sin `exit()` — falso verde en runner agrupado | `77713c8` | Añadido `exit(TestSuite::exitCode());` a `HookFilterTest.php` y `HookFilterApiTest.php`. El tercer fichero original (`PluginHookRegistryTableTest.php`) se eliminó por completo en `03.01` (la tabla que probaba dejó de existir), así que ya no aplica. Test nuevo `TestRunnerExitCodeTest.php`: lee `run.php`, extrae la lista de ficheros registrados y comprueba que estos dos terminan exactamente en `TestSuite::summary(); exit(TestSuite::exitCode());`. Deliberadamente acotado a estos dos ficheros — `CommentsPluginTest.php`, `EntityDataTableTest.php` y `GenericRepositoryTest.php` tienen el mismo hueco pero están fuera de alcance de este hallazgo (lo dice el propio informe), y barrerlos aquí habría chocado con hallazgos de otros ficheros de la auditoría. Confirmado por reversión manual: el test nuevo falla listando los 2 ficheros si se revierte el `exit()` de cualquiera de los dos. Verificado con `unit` + `integration-db` + `integration-plugins` completos: 0 fallos (salvo el preexistente ya anotado en `03.01`). |
