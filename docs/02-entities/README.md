...
El sistema obtiene el listado de entidades funcionales exclusivamente desde
la tabla `plugins`, filtrando por `manifest_json->>'type' = 'entity'` y
`status = 'active'`. Toda la lógica de alta, consulta y
validación de entidades se basa en los plugins instalados y activos.
